...
\begin{itemize}[leftmargin=4.5mm,itemsep=1.5pt,topsep=1.5pt,partopsep=0pt,parsep=0pt]
    \item \textbf{Milestone 1 (Phase 1):} Core Data Structure Implementation — Build C modules for Hash Tables (keyword indexing), Linked Lists (news records), and Binary Search Trees (sorting and filtering records by score and date).
    ...
    \item \textbf{Milestone 5 (Phase 3):} Testing \& Comprehensive Benchmarking — Unit test matching accuracy, handle edge cases, and generate final proposal deliverables.
\end{itemize}
\end{dynamicanswerbox}
\vspace{10mm}
...
\begin{dynamicanswerbox}
We are implementing this project using a modular, multi-tier architecture that combines low-level C data structures for optimal performance with high-level C++ object-oriented programming (OOP) for maintainability and modular design:

\vspace{1mm}
\begin{itemize}[leftmargin=4.5mm,itemsep=1.5pt,topsep=1.5pt,partopsep=0pt,parsep=0pt]
    \item \textbf{Text Processing \& Keyword Extraction:} Clean input text, remove unnecessary elements, filter stop words, and extract key entities.
    \item \textbf{Data Structures \& Matching Layer (C):} Hash Tables are used for quick keyword lookups in near constant time, dynamic Linked Lists are used to chain records, Binary Search Trees (BST) are used for filtering based on time or score ranges, FIFO Queues are used for grouping input data, and Stacks are used to track operation history.
    \item \textbf{Multi-Factor Verification Engine:} This system calculates an overall score based on several factors: Entity Match (40\%), Source Reliability (25\%), Corroborating Reports (20\%), Temporal Consistency (10\%), and Category Alignment (5\%).
    \item \textbf{Classification \& Persistence (C++ OOP):} Output is categorized into three groups: Likely Verified (80–100\%), Needs Verification (50–79\%), or Suspicious (0–49\%).
\end{itemize}
\end{dynamicanswerbox}
...
\begin{itemize}[leftmargin=4.5mm,itemsep=1pt,topsep=1pt,partopsep=0pt,parsep=0pt]
    \item Complete, modular, and well-documented source code that combines custom C data structures (Hash Tables, BST, Linked Lists, Queues, Stacks) with C++ OOP classes.
    \item A pre-configured dataset with verified news articles, flagged suspicious records, and indexes for trusted publishers.
    \item An interactive Administrative module that provides full Create, Read, Update, and Delete (CRUD) functionalities for managing the database and viewing audit history.
    \item A comprehensive project report, analysis of algorithmic complexity, UML diagrams, and a live demonstration showing real-time news verification and scoring.
\end{itemize}
\end{dynamicanswerbox}
\vspace{7mm}
...
\begin{dynamicanswerbox}
All submissions are assumed to be in English and contain enough entities for tokenization.
A baseline truth dataset is managed by administrators.The tool is designed as a fast, deterministic local verification tool, not an exhaustive real-time crawler.
\end{dynamicanswerbox}
\vspace{7mm}
